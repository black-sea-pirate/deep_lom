export default {
  // Common
  common: {
    welcome: "Добро пожаловать",
    login: "Войти",
    logout: "Выйти",
    register: "Зарегистрироваться",
    email: "Электронная почта",
    password: "Пароль",
    firstName: "Имя",
    lastName: "Фамилия",
    name: "Имя",
    status: "Статус",
    actions: "Действия",
    save: "Сохранить",
    cancel: "Отмена",
    delete: "Удалить",
    edit: "Редактировать",
    create: "Создать",
    search: "Поиск",
    loading: "Загрузка...",
    back: "Назад",
    next: "Далее",
    finish: "Завершить",
    submit: "Отправить",
    confirm: "Подтвердить",
    reject: "Отклонить",
    selected: "Выбрано",
    yesterday: "Вчера",
    daysAgo: "дней назад",
    // Test review page
    summary: "Итоги",
    student: "Студент",
    points: "Баллы",
    answers: "Ответы",
    studentAnswer: "Ответ студента",
    correctAnswer: "Правильный ответ",
    feedback: "Комментарий",
    or: "или",
  },

  // Navigation
  nav: {
    dashboard: "Главная панель",
    projects: "Проекты",
    tests: "Тесты",
    statistics: "Статистика",
    settings: "Настройки",
  },

  // Auth
  auth: {
    loginTitle: "Вход",
    registerTitle: "Создать аккаунт",
    forgotPassword: "Забыли пароль?",
    noAccount: "Нет аккаунта?",
    hasAccount: "Уже есть аккаунт?",
    signIn: "Войти",
    signUp: "Зарегистрироваться",
    role: "Роль",
    teacher: "Преподаватель",
    student: "Студент",
    // Password reset
    resetPassword: "Сброс пароля",
    resetStep1Title: "Забыли пароль?",
    resetStep1Desc: "Введите email — мы отправим 6-значный код.",
    resetStep2Title: "Введите код",
    resetStep2Desc: "Проверьте входящие письма.",
    resetStep3Title: "Новый пароль",
    sendCode: "Отправить код",
    verifyCode: "Подтвердить",
    codeSent: "Код отправлен! Проверьте почту.",
    codeLabel: "6-значный код",
    newPassword: "Новый пароль",
    confirmNewPassword: "Повторите пароль",
    passwordsDoNotMatch: "Пароли не совпадают",
    resetSuccess: "Пароль изменён! Войдите с новым паролем.",
    backToLogin: "Вернуться ко входу",
    // Email verification
    verifyEmailTitle: "Подтвердите Email",
    verifyEmailDesc: "Мы отправили 6-значный код на {email}. Введите его ниже.",
    verifyEmailCode: "Код подтверждения",
    verifyEmailBtn: "Подтвердить Email",
    verifyEmailResend: "Отправить повторно",
    verifyEmailResendWait: "Повторная отправка через {sec}с",
    verifyEmailSuccess: "Email подтверждён! Добро пожаловать.",
    verifyEmailSkip: "Подтвердить позже",
  },

  // Teacher Dashboard
  teacher: {
    menu: {
      projects: "Проекты",
      participants: "Участники",
      materials: "Материалы",
      lobby: "Проведение / Лобби",
      analytics: "Аналитика и отчёты",
      settings: "Настройки",
    },
    dashboard: "Панель Преподавателя",
    createProject: "Создать Новый Проект",
    myProjects: "Мои Проекты",
    projectTitle: "Название Проекта",
    groupName: "Название Группы",
    description: "Описание",
    uploadMaterials: "Загрузить Материалы",
    generateTests: "Сгенерировать Тесты",
    viewTests: "Просмотреть Тесты",
    studentCount: "Студенты",
    status: "Статус",
    lobby: "Лобби",
    startTest: "Начать Тест",
    editQuestions: "Редактировать Вопросы",
    projectSettings: "Настройки Проекта",
    timerMode: "Режим таймера",
    totalTimeMode: "Общее время",
    perQuestionMode: "На вопрос",
    totalTimeModeHint: "Установить единый лимит времени на весь тест",
    perQuestionModeHint: "Установить лимит времени на каждый вопрос отдельно",
    totalTime: "Общее время",
    timePerQuestion: "Время на вопрос",
    timePerQuestionHint: "Настройте время для каждого типа вопросов ниже",
    minutes: "мин",
    seconds: "сек",
    maxStudents: "Максимум студентов",
    numVariants: "Варианты теста",
    numVariantsHint:
      "Количество уникальных версий теста для защиты от списывания",
    testLanguage: "Язык теста",
    testLanguageHint:
      "Язык, на котором будут сгенерированы вопросы (независимо от языка материалов)",
    langEnglish: "Английский",
    langRussian: "Русский",
    langUkrainian: "Украинский",
    langPolish: "Польский",
    questionTypes: "Типы вопросов",
    singleChoice: "Один Вариант",
    multipleChoice: "Несколько Вариантов",
    trueFalse: "Правда/Ложь",
    shortAnswer: "Краткий Ответ",
    essay: "Эссе",
    matching: "Сопоставление",
    questionCount: "Количество Вопросов",
    waitingStudents: "Ожидающие Студенты",
    onlineNow: "Онлайн Сейчас",
    waitingForStudents: "Пока нет студентов онлайн. Ожидаем подключения...",
    readyToStart: "Готово к Началу",
    addStudent: "Добавить Студента",
    removeStudent: "Удалить Студента",
    studentEmail: "Email Студента",
    selectStudent: "Выберите студента из контактов",
    noConfirmedContacts: "Нет подтверждённых контактов",
    noContactsTitle: "Нет подтверждённых контактов",
    noContactsDesc:
      "Добавьте студентов в контакты в разделе Участники и дождитесь их подтверждения.",
    addStudentFirst: "Добавьте хотя бы одного студента перед началом теста",
    testStarted: "Тест начат! Все студенты теперь могут приступить.",
    testStartFailed: "Не удалось начать тест",
    testInProgress: "Тест в процессе",
    testCompleted: "Тест завершён",
    restartTest: "Перезапустить Тест",
    scheduleTest: "Запланировать Тест",
    startTime: "Время Начала",
    endTime: "Время Окончания",
    activateTest: "Активировать Тест",
    testSchedule: "Расписание Теста",
    availableFrom: "Доступен С",
    availableUntil: "Доступен До",
    viewStatistics: "Просмотреть Статистику",
    completedTests: "Завершенные Тесты",
    studentResults: "Результаты Студентов",
    darkMode: "Темная Тема",
    // Titles for newly added teacher pages
    participants: "Участники",
    materials: "Материалы",
    generation: "Генерация тестов",
    lobbyHub: "Проведение / Лобби",
    analyticsReports: "Аналитика и отчёты",
    settings: "Настройки",
  },

  // Lobby view translations
  lobby: {
    testResults: "Результаты тестирования",
    completed: "завершено",
    inProgress: "в процессе",
    testStatus: "Статус теста",
    score: "Баллы",
    timeTaken: "Время",
    grading: "Проверка",
    viewAnswers: "Просмотреть ответы",
    downloadPDF: "Скачать PDF",
    deleteResults: "Удалить результаты",
    notStartedYet: "Ещё не начал",
    noResultsYet: "Результатов пока нет",
    statusPending: "Ожидание",
    statusInProgress: "В процессе",
    statusCompleted: "Завершено",
    statusGraded: "Проверено",
    statusNotStarted: "Не начато",
    notStarted: "Не начал",
    confirmDeleteResults:
      "Вы уверены, что хотите удалить все результаты тестирования для",
    confirmDeleteWarning:
      "Это действие нельзя отменить. Все данные теста будут удалены безвозвратно.",
    generatingPDF: "Генерация PDF...",
    pdfDownloaded: "PDF успешно скачан",
    pdfError: "Не удалось сгенерировать PDF",
    deleteSuccess: "Результаты теста успешно удалены",
    studentAnswers: "Ответы студента",
    selectGroup: "Выберите группу",
    addGroup: "Добавить группу",
    noGroups: "Нет доступных групп",
    pendingGrading: "Ожидание проверки ИИ",
    graded: "Проверено",
  },

  // Teacher Settings
  teacherSettings: {
    title: "Настройки аккаунта",
    profile: "Профиль",
    security: "Безопасность",
    changePassword: "Изменить пароль",
    currentPassword: "Текущий пароль",
    newPassword: "Новый пароль",
    confirmNewPassword: "Подтвердите новый пароль",
    updatePassword: "Обновить пароль",
    passwordChanged: "Пароль успешно изменён",
    passwordMismatch: "Пароли не совпадают",
    passwordTooShort: "Пароль должен содержать минимум 6 символов",
    wrongCurrentPassword: "Неверный текущий пароль",
    firstName: "Имя",
    lastName: "Фамилия",
    email: "Email",
    updateProfile: "Обновить профиль",
    profileUpdated: "Профиль успешно обновлён",
    emailCannotChange: "Email адрес нельзя изменить",
  },

  // Student Dashboard
  student: {
    dashboard: "Панель Студента",
    upcomingTests: "Предстоящие Тесты",
    completedTests: "Завершенные Тесты",
    myStatistics: "Моя Статистика",
    averageScore: "Средний Балл",
    totalTests: "Всего Тестов",
    takeTest: "Пройти Тест",
    viewResults: "Просмотреть Результаты",
    joinTest: "Присоединиться к Тесту",
    testInProgress: "Тест в Процессе",
    timeRemaining: "Осталось Времени",
    question: "Вопрос",
    of: "из",
    submitAnswer: "Отправить Ответ",
    previousQuestion: "Предыдущий",
    nextQuestion: "Следующий",
    submitTest: "Отправить Тест",
    noUpcomingTests: "Нет предстоящих тестов",
    // Matching questions
    matchingInstructions:
      "Сопоставьте каждый элемент слева с правильным элементом справа",
    selectMatch: "Выберите соответствие",
    // Countdown timer
    available: "Доступен",
    scheduled: "Запланирован",
    startsIn: "Начнётся через",
    endsIn: "Закончится через",
    testEnded: "Тест завершён",
    availableNow: "Доступен сейчас",
    waitingForStart: "Ожидание начала",
    // Notifications / Contact requests
    notifications: "Уведомления",
    contactRequestsInfo:
      "Здесь отображаются запросы от преподавателей на добавление вас в контакты",
    teacherEmail: "Email преподавателя",
    yourNameInRequest: "Ваше имя в запросе",
    yourEmailInRequest: "Ваш email в запросе",
    youConfirmed: "Вы подтвердили",
    noContactRequests: "Нет запросов на подтверждение",
    requestConfirmed: "Запрос подтверждён",
  },

  // Student Account
  studentAccount: {
    manageEmails: "Управление email",
    changePassword: "Изменить пароль",
    yourEmails: "Ваши email адреса",
    emailsHint:
      "Добавьте email из разных учреждений для получения приглашений на тесты.",
    primary: "Основной",
    makePrimary: "Сделать основным",
    addNewEmail: "Добавить новый email",
    addEmail: "Добавить email",
    institution: "Учреждение",
    emailPlaceholder: "ваш{'@'}email.com",
    institutionPlaceholder: "Название учреждения",
    emailAdded: "Email успешно добавлен",
    emailRemoved: "Email удалён",
    primaryChanged: "Основной email изменён",
    confirmRemoveEmail: "Вы уверены, что хотите удалить этот email?",
    cannotRemovePrimary: "Невозможно удалить основной email",
    invalidEmail: "Введите корректный email адрес",
    currentPassword: "Текущий пароль",
    newPassword: "Новый пароль",
    confirmNewPassword: "Подтвердите новый пароль",
    passwordChanged: "Пароль успешно изменён",
    passwordMismatch: "Пароли не совпадают",
    passwordTooShort: "Пароль должен быть не менее 6 символов",
  },

  // Student Statistics
  studentStats: {
    totalDescription:
      "Общее количество тестов, назначенных вам по всем курсам.",
    averageDescription: "Ваш средний балл по всем завершённым тестам.",
    testName: "Название теста",
    course: "Курс",
    date: "Дата",
  },

  // Project
  project: {
    statusDraft: "Черновик",
    statusReady: "Готов",
    statusActive: "Активен",
    statusCompleted: "Завершён",
    statusGenerating: "Генерация",
    statusVectorizing: "Векторизация",
    questions: "Вопросы",
    editQuestions: "Редактировать вопросы",
    activate: "Активировать",
    complete: "Завершить",
    createdAt: "Создан",
    confirmActivate: "Активировать проект? Студенты смогут проходить тест.",
    confirmComplete:
      "Завершить проект? Студенты больше не смогут проходить тест.",
    confirmDelete: "Удалить проект? Это действие нельзя отменить.",
    activated: "Проект успешно активирован",
    completed: "Проект успешно завершён",
    deleted: "Проект успешно удалён",
    noMaterials: "Материалы не прикреплены",
    schedule: "Расписание",
    startTime: "Время начала",
    endTime: "Время окончания",
    editProject: "Редактировать проект",
    editSettings: "Редактировать настройки проекта",
    updated: "Проект успешно обновлён",
  },

  // Project Wizard
  wizard: {
    step1: "Информация о Проекте",
    step2: "Выбрать Материалы",
    step3: "Настроить Параметры",
    step4: "Сгенерировать Тесты",
    projectInfo: "Введите информацию о проекте",
    uploadFiles: "Загрузите учебные материалы",
    dragDrop: "Перетащите файлы сюда",
    orBrowse: "или выберите",
    supportedFormats: "Поддерживаемые форматы: PDF, DOCX, TXT, PNG, JPG",
    configureSettings: "Настройте параметры теста",
    generatingTests: "Генерация тестов...",
    testsGenerated: "Тесты успешно сгенерированы!",
    readyToTest: "Проект создан и готов к тестированию!",
    goToDashboard: "Перейти на главную",
    selectMaterials: "Выбрать материалы",
    selectMaterialsHint: "Выберите материалы из библиотеки для генерации теста",
    selected: "выбрано",
    rootLevel: "Корневой уровень",
    noMaterialsHere: "Нет материалов в этом месте",
    goToMaterials: "Перейти к материалам",
  },

  // Results
  results: {
    title: "Результаты Теста",
    testResults: "Результаты Теста",
    loading: "Загрузка результатов...",
    score: "Балл",
    yourScore: "Ваш Балл",
    correctAnswers: "Правильные Ответы",
    incorrectAnswers: "Неправильные Ответы",
    feedback: "Обратная Связь",
    yourAnswer: "Ваш Ответ",
    correctAnswer: "Правильный Ответ",
    detailedResults: "Подробные Результаты",
    passed: "Сдано",
    failed: "Не сдано",
    backToDashboard: "Вернуться на Главную",
    testInfo: "Информация о Тесте",
    projectTitle: "Проект",
    status: "Статус",
    startedAt: "Начат",
    completedAt: "Завершён",
    questions: "Вопросов",
    question: "Вопрос",
    noAnswer: "Ответ не предоставлен",
    true: "Верно",
    false: "Неверно",
    aiGraded: "Оценено ИИ",
    // Question types
    singleChoice: "Один Ответ",
    multipleChoice: "Множественный Выбор",
    trueFalse: "Верно/Неверно",
    shortAnswer: "Короткий Ответ",
    essay: "Эссе",
    matching: "Сопоставление",
    // AI Grading
    aiGrading: "Оценка ИИ",
    aiGradingInProgress: "ИИ Оценивает Ответы",
    aiGradingPendingDesc:
      "Некоторые ответы ещё оцениваются. Результаты обновятся автоматически.",
    gradedBy: "Проверено",
    gradingInProgress: "Идёт проверка...",
    criteriaScores: "Критерии Оценки",
    keyStrengths: "Сильные Стороны",
    areasForImprovement: "Области для Улучшения",
    detectedKeywords: "Обнаруженные Ключевые Слова",
    gradingStatus: {
      pending: "Ожидание",
      in_progress: "В процессе",
      completed: "Завершено",
      failed: "Ошибка",
    },
  },

  // Status
  status: {
    draft: "Черновик",
    ready: "Готов",
    active: "Активный",
    completed: "Завершен",
    pending: "Ожидание",
    inProgress: "В Процессе",
    graded: "Оценен",
  },

  // Materials Page
  materialsPage: {
    title: "Материалы",
    uploadNew: "Загрузить",
    noMaterials: "Материалы еще не загружены",
    uploadFirst: "Загрузите первый учебный материал",
    fileName: "Название файла",
    fileType: "Тип",
    fileSize: "Размер",
    uploadDate: "Загружен",
    actions: "Действия",
    preview: "Просмотр",
    download: "Скачать",
    delete: "Удалить",
    confirmDelete: "Вы уверены, что хотите удалить этот материал?",
    uploadSuccess: "Материал успешно загружен",
    uploadError: "Ошибка загрузки материала",
    deleteSuccess: "Материал успешно удален",
    deleteError: "Ошибка удаления материала",
    dragDropHint: "Перетащите файлы сюда или нажмите для выбора",
    supportedFormats: "Поддерживаются: PDF, DOCX, TXT, PNG, JPG (макс. 50МБ)",
    uploading: "Загрузка...",
    processing: "Обработка...",
    // Folder management
    folders: "Папки",
    files: "Файлы",
    createFolder: "Создать папку",
    editFolder: "Редактировать папку",
    folderName: "Название папки",
    folderNameRequired: "Название папки обязательно",
    folderNamePlaceholder: "Введите название папки",
    folderDescription: "Описание",
    folderDescriptionPlaceholder: "Необязательное описание",
    folderCreated: "Папка создана",
    folderUpdated: "Папка обновлена",
    folderDeleted: "Папка удалена",
    confirmDeleteFolder: "Удалить папку",
    noFolders: "Папок пока нет. Создайте папку для организации материалов.",
    moveToFolder: "Переместить в папку",
    selectDestination: "Выберите папку назначения для",
    rootFolder: "Корень (без папки)",
    current: "Текущая",
    fileMoved: "Файл перемещен",
    uploadTo: "Загрузить в:",
  },

  // Participants Page
  participantsPage: {
    title: "Участники",
    searchStudents: "Поиск студентов...",
    addStudent: "Добавить студента",
    editStudent: "Редактировать студента",
    createGroup: "Создать группу",
    editGroup: "Редактировать группу",
    totalStudents: "Всего студентов",
    totalGroups: "Групп",
    individualStudents: "Индивидуальные",
    allStudents: "Все студенты",
    groups: "Группы",
    name: "Имя",
    email: "Email",
    type: "Тип",
    group: "Группа",
    addedOn: "Добавлен",
    individual: "Индивидуальный",
    groupMember: "Член группы",
    noStudents: "Студентов пока нет",
    addFirstStudent: "Добавьте первого студента для начала",
    noGroups: "Групп пока нет",
    createFirstGroup: "Создайте группу для организации студентов",
    members: "Участники",
    created: "Создана",
    firstName: "Имя",
    lastName: "Фамилия",
    firstNamePlaceholder: "Иван",
    lastNamePlaceholder: "Иванов",
    emailPlaceholder: "student{'@'}university.edu",
    studentType: "Тип студента",
    selectGroup: "Выбрать группу",
    chooseGroup: "Выберите группу",
    groupName: "Название группы",
    groupNameRequired: "Название группы обязательно",
    groupNamePlaceholder: "например, ИС-101 Секция А",
    description: "Описание",
    descriptionPlaceholder: "Необязательное описание группы",
    fillRequired: "Заполните все обязательные поля",
    invalidEmail: "Введите корректный email",
    duplicateEmail: "Этот email уже зарегистрирован",
    studentAdded: "Студент добавлен",
    studentUpdated: "Студент обновлен",
    studentDeleted: "Студент удален",
    confirmDeleteStudent: "Удалить студента",
    groupCreated: "Группа создана",
    groupUpdated: "Группа обновлена",
    groupDeleted: "Группа удалена",
    confirmDeleteGroup: "Удалить группу",
    autoFillFromDb: "Автозаполнение из базы данных",
    status: "Статус",
    confirmed: "Подтверждён",
    pending: "Ожидает",
    rejected: "Отклонён",
    studentFound: "Найден",
    studentFoundInDb: "Студент найден в базе данных",
    studentNotFoundInDb:
      "Студент не найден в базе данных. Введите имя вручную.",
    loadError: "Не удалось загрузить студентов",
    loadGroupsError: "Не удалось загрузить группы",
  },

  // Analytics Page
  analyticsPage: {
    title: "Аналитика и отчёты",
    showDataFor: "Показать данные за:",
    totalTests: "Всего тестов",
    totalStudents: "Всего студентов",
    averageScore: "Средний балл",
    completionRate: "Процент завершения",
    avgTime: "Ср. время",
    thisMonth: "в этом месяце",
    new: "новых",
    scoreDistribution: "Распределение оценок",
    recentTests: "Недавние тесты",
    topStudents: "Лучшие студенты",
    projectPerformance: "Эффективность проектов",
    project: "Проект",
    date: "Дата",
    students: "Студенты",
    avgScore: "Ср. балл",
    passRate: "% сдавших",
    tests: "Тесты",
    trend: "Тренд",
  },

  // Question Editor
  question: {
    add: "Добавить вопрос",
    edit: "Редактировать вопрос",
    delete: "Удалить вопрос",
    variant: "Вариант теста",
    variantNumber: "Вариант {n}",
    questionsCount: "вопросов",
    noQuestions:
      "Вопросов пока нет. Добавьте вопросы вручную или сгенерируйте с помощью AI.",
    created: "Вопрос создан",
    updated: "Вопрос обновлён",
    deleted: "Вопрос удалён",
    confirmDelete: "Вы уверены, что хотите удалить этот вопрос?",
    type: "Тип вопроса",
    text: "Текст вопроса",
    points: "Баллы",
    options: "Варианты ответа",
    addOption: "Добавить вариант",
    correctAnswer: "Правильный ответ",
    expectedKeywords: "Ожидаемые ключевые слова",
  },
};
